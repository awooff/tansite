...
const hack = {
  ...
  delay: async (
    ...
    return (
      hasher.getExecutionCost("execute") +
      cracker.getExecutionCost("execute") +
      settings.processDelay.hack * 1000
    );
  },
  before: async (
    computer: Computer | null,
    ...
    if (hasher.level > cracker.level) {
      throw new GameException("your cracker is too weak");
    }
